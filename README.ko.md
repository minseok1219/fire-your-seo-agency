...
> 포크입니다. 원본의 다섯 레인에 **SPO(네이버 스마트플레이스)·KMO(카카오맵) 레인**과
> 진단 스크립트를 추가했습니다. 오프라인 업장(체육관·식당·병원·학원)이라면 이 포크가 맞습니다.
...
같은 "검색 최적화"라도 상대하는 엔진이 다릅니다. 이 스킬은 일곱 레인을 구분해서 각각 최적화합니다.
...
| **KMO** (Kakao Map) | 카카오맵·카카오톡 검색 | 카카오맵 카드가 네이버와 글자 단위로 일치하고, 리뷰·소식이 비어 있지 않은가? |
...
**KMO는 네이버와 맞추는 레인입니다.** 카카오맵은 대부분 등록만 하고 방치되어 업체명 띄어쓰기,
영업시간 분 단위, 공휴일 정책, 가격표가 네이버와 어긋나 있습니다. `kakao-audit.py --naver`가
두 채널을 필드별로 대조해 불일치 목록을 뽑고, 비어 있는 카카오맵 리뷰·소식·톡채널을 채우게 합니다.
...
/fire-your-seo-agency 카카오맵이랑 네이버 정보 맞춰줘
```
...
7. **카카오맵** — 네이버와 필드별 대조(불일치 0건), 카카오맵 리뷰·소식 채우기, 톡채널·카카오 예약
8. **측정 루프** — 고치고 끝이 아니라, 재측정 일정을 잡고 숫자로 확인합니다
...
references/
  ...
  kmo-kakaomap.md     ← 카카오맵 (네이버 대조·리뷰·소식 복제·톡채널·카카오 예약)  ★ 포크 추가
  measure.md          ← 측정 루프 (고치고 끝이 아니다)
  en/                 ← 전체 레퍼런스 영문 미러 (사람 독자용)
scripts/
  place-audit.py      ← 네이버 플레이스 공개 페이지를 크롤러의 눈으로 읽는 진단 스크립트 (표준 라이브러리 + curl)  ★ 포크 추가
  kakao-audit.py      ← 카카오맵 공개 API 진단 + --naver 로 네이버와 필드별 대조  ★ 포크 추가
