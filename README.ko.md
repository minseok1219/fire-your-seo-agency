...
대신 공개 플레이스 페이지가 SSR로 내려주는 데이터를 읽어 **영업시간·가격표·리뷰·소식·전환 경로**를
진단합니다. 로그인·API 키 없이 `naver.me` 링크 하나면 됩니다.
...
6. **스마트플레이스** — 영업시간·주소 완비, 소개글 직답, 가격표, 예약·톡톡·전화 전환 경로, 소식 리듬
...
references/
  ...
  spo-smartplace.md   ← 스마트플레이스 (영업시간·소개글·가격·리뷰·소식·전환·NAP 일관성)  ★ 포크 추가
